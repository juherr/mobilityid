...
- The open-ended single-valued identifiers (`PhoneCountryCode`, `ProviderId`, `OperatorIdIso`,
  `OperatorIdDin`) are [branded strings](https://www.learningtypescript.com/articles/branded-types):
  `ProviderId.from("tnm")` returns the plain string `"TNM"` typed as `ProviderId`, a type that
  only the factory can produce. There is no wrapper object, so `===`, `JSON.stringify`, template
  literals and `Map`/`Set` keys work on the value itself, while `const id: ProviderId = "TNM"` is
  a compile error. The brand widens to `string` for free; the reverse only goes through `from`,
  `parse` or `tryParse`: there is no runtime `instanceof`, and `isValid` only answers whether
  `from` would accept the input (`isValid("tnm")` is `true`) without narrowing it, so parse and
  use the returned value where a `ProviderId` is required. `src/string-id.ts` holds the `StringId` type and the `defineStringId`
  helper that builds the four entry points once for every such identifier.
- `CountryCode` is a closed set, so it is a literal union of the 249 ISO 3166-1 alpha-2 codes
  instead of a brand: `const cc: CountryCode = "NL"` type-checks with completion, `"XX"` and the
  lowercase `"nl"` do not, and a `switch` over codes is exhaustiveness-checkable. The companion has
  the same four entry points, and `CountryCode.from("nl")` still normalizes to `"NL"`: external
  input goes through the parser. The list (`src/iso3166-alpha2.ts`, also exported as
  `ISO_3166_ALPHA2`) is generated by `scripts/generate-iso3166.sh` from the JDK's
  `Locale.getISOCountries()`, the source the Scala, Java and Go ports validate against, so the
...
- Foundational identifiers: `CountryCode` (literal union), `PhoneCountryCode`, `ProviderId`, `OperatorIdIso`, `OperatorIdDin` (branded strings), `PartyId`
