...
- Security gates: `dependency-submission.yml` submits the resolved Gradle graph (GitHub cannot parse Gradle) on `main` and uploads it as an artifact on pull requests (no pull request run holds a write token); dependency review on pull requests (same workflow, manifest ecosystems, fails on high+ in any scope); `trusted-dependency-review.yml` (`workflow_run` from `main`) submits the uploaded graph of every PR after provenance validation, recomputes the review and publishes the `Trusted dependency review` commit status through a dedicated GitHub App whose key lives in the `trusted-review` environment (branch policy `main`), the check to require, with "Require branches to be up to date" as prerequisite; `pull-request-lifecycle.yml` (`pull_request_target`, metadata only) re-runs `Dependency Submission` when a PR is retargeted, reopened or closed so the status follows; write collaborators are trusted by definition (they can submit snapshots directly), `.github/CODEOWNERS` covers `.github/` and `scripts/` (see `CONTRIBUTING.md`); Dependabot alerts on `main`; weekly/on-demand OWASP Dependency-Check full scan (`security.yml`, not a PR gate).
