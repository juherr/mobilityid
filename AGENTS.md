...
- `Release` (`release.yml`) is dispatched manually from `main` with the version as input: it checks `CHANGELOG.md` and `.github/release-notes/X.Y.Z.md`, runs the Java, TypeScript, PHP, Go and Scala preflights, and only when all pass publishes Java (Maven Central Portal via nmcp), Scala (Maven Central Portal via sbt `publishSigned` + `sonaRelease`, Scala 2.13 and 3), TypeScript (npm via Trusted Publishing/OIDC, no token) and PHP (a `git subtree split` of `php/` pushed to the `juherr/mobility-id-php` mirror that Packagist follows, via a deploy key) idempotently, then creates the signed `vX.Y.Z` and `go/vX.Y.Z` tags (Go has no registry: the tag is its release, fetched by proxy.golang.org) and the GitHub Release. Full procedure in `CONTRIBUTING.md`.
...
- Security gates: `dependency-submission.yml` submits the resolved Gradle graph (GitHub cannot parse Gradle) on `main` and uploads it on pull requests; `trusted-dependency-review.yml` (`workflow_run` from `main`) submits and reviews it for every PR and publishes the `Trusted dependency review` commit status, the check to require; `pull-request-lifecycle.yml` keeps that status in step with retargets, reopens and closes. Threat model, setup and post-merge checks: `CONTRIBUTING.md`, "Dependency review gate". Dependabot alerts on `main`; weekly/on-demand OWASP Dependency-Check full scan (`security.yml`, not a PR gate).
