...
- Security gates: `dependency-submission.yml` submits the resolved Gradle graph (GitHub cannot parse Gradle) on `main` and same-repository PRs; dependency review on pull requests (same workflow, job after the submission, fails on high+ in any scope); `trusted-dependency-review.yml` (`workflow_run` from `main`) recomputes the review for every PR and publishes the `Trusted dependency review` commit status through a dedicated GitHub App whose key lives in the `trusted-review` environment (branch policy `main`), the check to require; it also submits the Java graph of fork and Dependabot PRs after provenance validation (see `CONTRIBUTING.md`); Dependabot alerts on `main`; weekly/on-demand OWASP Dependency-Check full scan (`security.yml`, not a PR gate).
