...
- `consumer-smoke/` -> separate Gradle build consuming the published artifact (Java on the module path, Kotlin on the classpath with strict JSpecify); `scripts/` -> `verify.sh` (entry point), `verify-release-wiring.sh`, `verify-consumer.sh`.
...
- Spotless with palantir-java-format + Apache license header, Error Prone + NullAway + JSpecify,
  `javac -Xlint:all -Werror`, `javadoc -Xdoclint:all -Werror`, reproducible jars.
